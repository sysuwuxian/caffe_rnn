...
cv::Mat ReadImageToFasterCVMat(const string& filename){
  cv::Mat cv_img;
  cv::Mat cv_img_origin = ReadImageToCVMat(filename);
  int height = cv_img_origin.rows;
  int width = cv_img_origin.cols;

  int new_height = 0, new_width = 0;
  int shorter = std::min(height, width);
  double scale = 1.0;
  if(shorter == height){
     scale = 600.0 / height;
     new_height = 600;
...
vector<int> ReadFileToVector(const string& filename) {
	...
	if(!file.is_open()){
	  LOG(ERROR)<<filename<<" don't exist!";
	  return vec;
	}
	int element;
	while(file >> element){
